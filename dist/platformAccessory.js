...
class GrizzlEChargerAccessory {
    constructor(hap, log, accessory, api, initialStation) {
        ...
        this.api = api;
        this.statusServices = new Map(); // subtype → service
        ...
        for (const { status, name, subtype } of STATUS_SENSORS) {
            const svc = this.accessory.getServiceById(this.hap.Service.OccupancySensor, subtype) ||
    ...
    connectorStatus(station) {
        if (station.connectors.length > 0) {
            ...
            for (const preferred of ['Charging', 'Preparing', 'SuspendedEVSE', 'SuspendedEV', 'Finishing']) {
                if (station.connectors.some((c) => c.status === preferred))
                    return preferred;
            }
    ...
    isCharging(station) {
        if (station.connectors.length > 0)
            return station.connectors.some((c) => c.status === 'Charging');
        return station.status === 'Charging';
    }
    isFaulted(station) {
        if (station.connectors.length > 0) {
            return station.connectors.some((c) => c.status === 'Faulted' || (c.errorCode && c.errorCode !== 'NoError'));
        }
        return station.status === 'Faulted' || (!!station.errorCode && station.errorCode !== 'NoError');
    }
    isCarConnected(station) {
        if (station.connectors.length > 0)
            return station.connectors.some((c) => CAR_CONNECTED_STATUSES.has(c.status));
    ...
    updateStation(station) {
        ...
        if (nowFaulted !== wasFaulted) {
            ...
            this.outletService.updateCharacteristic(this.hap.Characteristic.StatusFault, nowFaulted ? this.hap.Characteristic.StatusFault.GENERAL_FAULT : this.hap.Characteristic.StatusFault.NO_FAULT);
        }
        if (nowCarConnected !== wasCarConnected) {
            this.log.info(`[${this.accessory.displayName}] Car ${nowCarConnected ? 'plugged in' : 'unplugged'}`);
            this.carPluggedInService.updateCharacteristic(this.hap.Characteristic.OccupancyDetected, this.occupancy(nowCarConnected));
        }
        if (nowStatus !== wasStatus) {
            ...
            for (const { status, subtype } of STATUS_SENSORS) {
                ...
                if (svc) {
                    svc.updateCharacteristic(this.hap.Characteristic.OccupancyDetected, this.occupancy(nowStatus === status));
                }
            }
        }
    }
